...
class NmapProcessWindows(Thread):
    """
    ...
    def __init__(self, targets="127.0.0.1",
        ...
        self.__nmap_binary_name = "nmap.exe"
    ...
    #     self.__sudo_run = "{0} -u {1}".format(sudo_path, sudo_user)
    #     super(NmapProcessWindows, self).start()
    ...
    def __wait(self):
        ...
        while (self.__nmap_proc.poll() is None or
            ...
            if not self.__ioerr_queue.empty():
                self.__stderr += str(self.__ioerr_queue.get_nowait(), "ascii")
            try:
                ...
                self.__stdout += str(thread_stream, "ascii")
